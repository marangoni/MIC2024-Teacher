/* ============================================================
   ...
   Versão 02 - atualização do bloco LED para inclusão do comando liga e comando desliga
...
// https://online.visual-paradigm.com/community/share/button-action-led-sdl-v2-vpd2-1mpipsstl2
//
...
#define   tick  (1<<PB5) // pino 13 tick - freq da interrupção TM2 - 1khz

#define   led1  (1<<PB2) // led 1 no pino D10 (PB2)
#define   led2  (1<<PB3) // led 2 no pino D11 (PB3)
...
switch (estado_CONTROL) {
  ...
  case 3:
    // Estado 3 - Aguarda sinal de termino da temporização do botao e detecta se acionamento foi curto ou longo
  if(msg_CRON1_done == 1){
    msg_CRON1_done = 0;	         //Consome o sinal
    
    // Detecta se acionamento foi curto ou longo
    if (CRON1_time > 1000){      //acionamento longo - função 1
      
      msg_LED1_on = 1;           //envia sinal ligar led 1
    }       
    else if (CRON1_time <= 1000){ //acionamento curto - função 2
      
      msg_LED2_on = 1;           //envia sinal ligar led 2
    } 
    // envia sinal para inicio da temporização do led
    msg_T1_start = 1;           //Envia sinal para iniciar timer 1
    
    estado_CONTROL = 4;         //vai para o estado 4

  }
    break;

  case 4:
    // Aguarda led1 ou led2 ligarem E sinal de termino da temporização 

    if((msg_T1_done == 1) && (f_LED1_status == 1)){

      msg_T1_done = 0;            // Consome o sinal

      msg_LED1_off = 1;           // Envia sinal para apagar o led 1

      estado_CONTROL = 1;         // Volta para o estado inicial

    }
      
    else if ((msg_T1_done == 1) && (f_LED2_status == 1)){
      msg_T1_done = 0;              // Consome o sinal

      msg_LED1_off = 1;             // Envia sinal para apagar o led 2
  ...
  switch (estado_BOTAO1){
    ...
    case 2:
      if ((PINC & bt_s1) == bt_s1 && f_botao_1_on){     // Se botao liberado (=1 pois é pull-up) apos ter sido pressionado...
         ...
         estado_BOTAO1 = 1;       //vai para o estado 1
      } 
    break;
  }
}
...
// Funcao de controle do led
  ...
  switch (estado_LED1) {
  case 1:
    // Estado 1 - aguarda sinal para ligar o led

   

    
    if (msg_LED1_on == 1 && f_LED1_status == 0) { //Se foi dado o comando para ligar e o led estiver desligado - ligar o led

     msg_LED1_on = 0;             //Consome o sinal para novo acionamento
    
     PORTB|= led1;                //Acende led
     f_LED1_status = 1;          //flag indica led ligado
     estado_LED1 = 2;              //Vai para estado 2
    }
    break;

    case 2:
    // Estado 2 - aguarda sinal para desligar o led
    
    if (msg_LED1_off == 1 && f_LED1_status == 1) { //Se foi dado o comando para desliga o led estiver ligado - desligar o led

    msg_LED1_off = 0;             //Consome o sinal para novo acionamento

    PORTB &= ~led1;              //Apaga o led
    f_LED1_status = 0;          //flag indica led ligado
    estado_LED1 = 1;              // Volta para estado 1
    }
    break;
  }
}
...
// Funcao de controle do led
  ...
  case 1:
    ...
    if (msg_LED2_on == 1 && f_LED2_status == 0) { //Se foi dado o comando para ligar e o led estiver desligado - ligar o led

     msg_LED2_on = 0;             //Consome o sinal para novo acionamento
    
     PORTB|= led2;                //Acende led
     f_LED2_status = 1;          //flag indica led ligado
    ...
    if (msg_LED2_off == 1 && f_LED2_status == 1) { //Se foi dado o comando para desliga o led estiver ligado - desligar o led

    msg_LED2_off = 0;             //Consome o sinal para novo acionamento

    PORTB &= ~led2;              //Apaga o led
    f_LED2_status = 0;          //flag indica led ligado
    
    estado_LED2 = 1;              // Volta para estado 1
    }
    break;
  }
}
